...
MSP.ConnectivityMatrixView.prototype = {
  ...
  draw: function (x, y) {
    ...
    if (typeof x !== "undefined" && typeof y !== "undefined") {
      ...
      context.rect(-(this.translateX / this.scale),
                   (y * (this.squareSize + this.squareMargin)) + this.squareMargin,
                   _SingletonConfig.width / this.scale,
                   this.squareSize);
      context.fill();
      context.beginPath();
      context.rect((x * (this.squareSize + this.squareMargin)) + this.squareMargin,
                   ...
                   this.squareSize,
                   _SingletonConfig.height / this.scale);
      context.fill();
    }
    ...
    for (var z = 0; z <= _SimulationData.gNeurons.length; z++) {
      ...
      context.beginPath();
      context.rect(0,
                   (z * (this.squareSize + this.squareMargin)),
                   _SimulationData.gNeurons.length * (this.squareSize + this.squareMargin),
                   this.squareMargin);
      context.fill();

      context.beginPath();
      context.rect((z * (this.squareSize + this.squareMargin)),
                   0,
                   this.squareMargin,
                   _SimulationData.gNeurons.length * (this.squareSize + this.squareMargin));
      context.fill();
    ...
    for (var k = 0; k < _SimulationData.gNeurons.length;
         k += Math.min(Math.max(Math.round(this.minAxisNumbers / this.scale), 1),
                       _SimulationData.gNeurons.length / this.minAxisNumbers)) {
      ...
      context.beginPath();
      context.rect(-((this.translateX - this.axisSize) / this.scale),
                   midPosX - (this.axisLineWidth * 0.5) / this.scale,
                   -this.axisLineHeight / this.scale,
                   this.axisLineWidth / this.scale);

      context.rect(midPosX - (this.axisLineWidth * 0.5) / this.scale,
    ...
    context.textBaseline = "middle";
    context.fillText("0,0", -((this.translateX - this.axisSize * 0.5) / this.scale),
                     -((this.translateY - this.axisSize * 0.5) / this.scale));
  ...
  mouseDown: function (self) {
    if (parseInt(d3.mouse(_SingletonConfig.svg.node())[0]) <= self.axisSize && parseInt(
        d3.mouse(_SingletonConfig.svg.node())[0]) <= self.axisSize) {
  ...
  mouseMove: function (self) {
    var color = rgbToHex(self.context.getImageData(parseInt(d3.mouse(_SingletonConfig.svg.node())[0]),
                                                   parseInt(d3.mouse(_SingletonConfig.svg.node())[1]), 1, 1).data);
    if (color === _SingletonConfig.EEColor || color === _SingletonConfig.IIColor
      ...
      self.draw(parseInt(parseInt((d3.mouse(_SingletonConfig.svg.node())[0] - self.translateX) / self.scale,
                                  10) / (self.squareSize + self.squareMargin)), parseInt(
        parseInt((d3.mouse(_SingletonConfig.svg.node())[1] - self.translateY) / self.scale,
                 10) / (self.squareSize + self.squareMargin)));
      ...
      d3.select("#tooltip")
        ...
        .html(
          "From: <b>" + _SimulationFilter.orderIndex[parseInt(
            parseInt((d3.mouse(_SingletonConfig.svg.node())[1] - self.translateY) / self.scale,
                     10) / (self.squareSize + self.squareMargin))]
          + "</b><br> To: <b>" + _SimulationFilter.orderIndex[parseInt(
            parseInt((d3.mouse(_SingletonConfig.svg.node())[0] - self.translateX) / self.scale,
                     10) / (self.squareSize + self.squareMargin))] + "</b>"
        );
  ...
  zoom: function (self) {
    if (!self.resetZoom) {
      self.translateX = Math.max(Math.min(d3.event.translate[0], self.axisSize),
                                 ((-_SimulationData.gNeurons.length * (self.squareSize + self.squareMargin) *
                                   d3.event.scale) + _SingletonConfig.width));
      self.translateY = Math.max(Math.min(d3.event.translate[1], self.axisSize),
                                 ((-_SimulationData.gNeurons.length * (self.squareSize + self.squareMargin) *
                                   d3.event.scale) + _SingletonConfig.height));
