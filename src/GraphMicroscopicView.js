...
MSP.GraphMicroscopicView.prototype = {
  ...
  generateGraph: function () {
    ...
    var ratioWidth = _SingletonConfig.width;
    var leftMargin = 60;
    var rightMargin = ratioWidth * 0.01;
    var width = _SingletonConfig.width * 0.49 - rightMargin - leftMargin;
    ...
    this.svg = d3.select("#renderArea")
                 ...
                 .attr("id", "caGraph")
                 .attr("width", _SingletonConfig.width * 0.49)
  ...
  updateGraph: function (selectedID) {
    ...
    neurons.forEach(function (d) {
      var color = "#ababab";
      if (typeof _SimulationController.view.selectedMicro !== "undefined" &&
          _SimulationController.view.selectedMicro.length === 0) {
        if ((typeof selectedID !== "undefined" && d === selectedID) ||
            (typeof selectedID === "undefined" && _SimulationFilter.gNeuronsFilterB[d]))
    ...
    neurons.forEach(function (d) {
      ...
      if (typeof _SimulationController.view.selectedMicro !== "undefined" &&
          _SimulationController.view.selectedMicro.length === 0) {
        if ((typeof selectedID !== "undefined" && d === selectedID) || (typeof selectedID === "undefined" &&
                                                                        _SimulationFilter.gNeuronsFilterB[d]))
    ...
    this.graph(1, this.svg, this.width, this.margin.left, this.margin.top, this.height, dataCalcium, false, 4,
               selectedID);
  ...
  graph: function (graphID, svg, widthTotal, marginLeft, marginTop, height, data, hasLegend, steps, selectedID) {
    ...
    x.domain([0, width]);
    var gBubbles = gGraph.append("g")
                         .attr("class", "history c" + graphID)
                         .attr("transform", "translate(0," + marginTop + ")");
    var gLines = gGraph.append("g")
                       .attr("class", "history b" + graphID)
                       .attr("transform", "translate(0," + marginTop + ")");
    var g = gGraph.append("g")
                  .attr("class", "history a" + graphID)
                  .attr("transform", "translate(0," + marginTop + ")");
    ...
    var numColors = _SingletonConfig.gSelectionIds.length;
