...
UI.Visualizer = function () {
  ...
  this.filterTypes = [{label: "Calcium Concentration", cat: "decimal", type: "Ca", color: "#de9425"},
    {label: "Excitatory Connections", cat: "real", type: "IConn", color: _SingletonConfig.IColor},
    {label: "Inhibitory Connections", cat: "real", type: "EConn", color: _SingletonConfig.EColor},
    {label: "Axonal Connections", cat: "real", type: "AConn", color: _SingletonConfig.AColor}];
...
UI.Visualizer.prototype = {
  ...
  createUIElements: function (self) {
    ...
    $("#jqxBottomControls_NumericInputStep").jqxNumberInput({
      width: '100px', height: '25px', inputMode: 'simple', spinButtons: true, decimalDigits: 0, min: 0, disabled: true
    });
    ...
    $("#numericBarElements").jqxNumberInput({
      width: '160px', height: '25px', inputMode: 'simple', spinButtons: true, decimalDigits: 0, min: 1, max: 20,
                                              value: 15
    });
    ...
    self.createColorCombos("#comboScaleTypeNeuron", self.colorScaleTypesElements, "#comboScaleNeuron",
                           self.colorScales);


    self.createColorCombos("#comboScaleTypeConnection", self.colorScaleTypesConnections, "#comboScaleConnection",
                           self.colorScales);
    ...
    self.createColorCombos("#comboScaleTypeCalcium", self.colorScaleTypesCalcium, "#comboScaleCalcium",
                           self.colorScales);
    ...
    $('#jqxMacroVControls_SliderApha').val(_SingletonConfig.macroVAlpha);
    ...
    $('#jqxWindow_Info').on('resize', function (event) {
  ...
  bindUIActions: function (self) {
    ...
    document.getElementById('fileDialog').addEventListener('change',
      function (evt) {
        ...
        if ((!files.length) || (files.length !== 6)) {
          ...
          _SingletonConfig.localFilesPath = $('#jqxInputSimulationFilesLocalPath').val();
    ...
    document.getElementById('fileDialogConfig').addEventListener('change',
      function (evt) {
        ...
        if ((!files.length) || (files.length !== 1)) {
          ...
          _SingletonConfig.localFilesPath = $('#jqxInputConfigFilesLocalPath').val();
    ...
    $("#btnAddID").on('click', function (event) {
      ...
      $("#centerEllipseList")
        .append('<button class="ellipseButton" onclick="delCenter(' + id + ');">' + id + '</button>');
    ...
    $("#btnAddSelected").on('click', function (event) {
      _SimulationFilter.gNeuronsFilter.forEach(function (z) {
        ...
        if (d.selected) {
          d.centerElipse = true;
          $("#centerEllipseList")
            .append('<button class="ellipseButton" onclick="delCenter(' + d.NId + ');">' + d.NId + '</button>');
        }
      });
      _SimulationController.view.update();
    });
    ...
    $("#jqxWindow_ButtonLoadRemoteSimulationFromDCache").on('click', function (event) {
      ...
      if (
        ...
        && (lUserLenght > 0)
        && (lPassLenght > 0)) {
        self.loadRemoteSimulationFromDCache(
          serversPath[$("#jqxWindow_LoadRemoteSimulation_ComboRemoteServers").selectedIndex],
    ...
    $("#btnApplyFilter").on('click', function () {
      ...
      $("#filterList").append(
        '<div class="filter"><div class="filterTitle" style="background-color:' + color + '">' +
        '<span class="filterTitleText">' + tipoCompleto + '</span><button class="filterButton">X</button> </div>' +
        '<div><span class="filterTitleSecondary">Applies to:</span><div class="filterContent">' +
        (filtro.excitatory ? '<div class="filterE"></div>' : "") +
        (filtro.inhibitory ? '<div class="filterI"></div>' : "") + '</div></div>' +
      ...
      $(".filterButton").last().on('click', function () {
    ...
    $("#comboScaleNeuron").on('change', function () {
      ...
      var colorConfig = [_SingletonConfig.EColor, _SingletonConfig.IColor, _SingletonConfig.AColor];
      ...
      _SingletonConfig.EColor = colorConfig[0];
      _SingletonConfig.IColor = colorConfig[1];
      _SingletonConfig.AColor = colorConfig[2];
      _SingletonConfig.neuronsScheme = colorScale;
      updateCookieColor();
    ...
    $("#comboScaleConnection").on('change', function () {
      ...
      var colorConfig = [
        _SingletonConfig.EEColor,
        _SingletonConfig.EIColor,
        _SingletonConfig.IEColor,
        _SingletonConfig.IIColor
      ];
      ...
      colorConfig = _ColorPicker.generateColors(colorScale, colorDOMObject, colorConfig, isCategorical);
      _SingletonConfig.EEColor = colorConfig[0];
      _SingletonConfig.EIColor = colorConfig[1];
      _SingletonConfig.IEColor = colorConfig[2];
      _SingletonConfig.IIColor = colorConfig[3];
      _SingletonConfig.connectionsScheme = colorScale;
    ...
    $("#comboScaleCalcium").on('change', function () {
      ...
      var colorConfig = [_SingletonConfig.EColor, _SingletonConfig.IColor];
      ...
      colorConfig = _ColorPicker.generateColors(colorScale, colorDOMObject, colorConfig, isCategorical);
      _SingletonConfig.minCaColor = colorConfig[0];
      _SingletonConfig.maxCaColor = colorConfig[1];
      _SingletonConfig.calciumScheme = colorScale;
      updateCookieColor();
      self.generateScaleCalcium();
      if (_SimulationController !== null) {
    ...
    $('#jqxMacroVControls_SliderApha').on('change', function (event) {
      _SingletonConfig.macroVAlpha = $('#jqxMacroVControls_SliderApha').val();
    });
    $("#jqxRadioButtonCaSetPointFromFile").on('change', function (event) {
      var checked = event.args.checked;
      if (checked) {
        _SingletonConfig.CaMaxMinValueTypes = 0;
      }
      _SimulationData.recalculateScales();
      _SimulationController.view.updateVisualization();
      self.createSampleBandColor();
    ...
    $("#jqxRadioButtonCaSetPointFromNeuronValues").on('change', function (event) {
      var checked = event.args.checked;
      if (checked) {
        _SingletonConfig.CaMaxMinValueTypes = 1;
      }
      _SimulationData.recalculateScales();
      _SimulationController.view.updateVisualization();
      self.createSampleBandColor();
    ...
    $("#dropDownSynapticElementsToViewButton").on("change", function () {
      _SingletonConfig.SEViewSelector = $(this).prop("selectedIndex");
      _SimulationController.view.updateVisualization();
    });
    ...
    $("#chkOutgoingConn").on('change', function (event) {
      _SingletonConfig.outgoingConn = event.args.checked;
      _SimulationController.view.updateVisualization();
    });
    $("#chkIncomingConn").on('change', function (event) {
      _SingletonConfig.incomingConn = event.args.checked;
      _SimulationController.view.updateVisualization();
    });
  ...
  _colorCallBack: function (originDOMElement, color, configPropID) {
    _SingletonConfig[configPropID] = color;
  ...
  generateScaleCalcium: function () {
    ...
    var colorRange = d4["interpolate" + _SingletonConfig.calciumScheme];
  ...
  closeNav: function () {
    ...
    document.getElementById("closeTabs").src = "./assets/images/menu.png";
    ...
    for (i = 0; i < y.length; i++) {
      ...
      $(".navOptionsButton img").eq(i)
                                .attr("src", $(".navOptionsButton img").eq(i).attr("src").replace("B.png", ".png"));
    }
  },
  ...
  generateCanvas: function (pMsg) {
    ...
    _SingletonConfig.svg.append("text")
      .style("fill", "black")
      .attr("x", _SingletonConfig.width / 2)
      .attr("y", _SingletonConfig.height / 2)
    ...
    if (_SimulationController !== null) {
      ...
      switch (pViewId) {
        ...
        case this.viewsID.MicroscopicView:
          //Constraint
          if (_SingletonConfig.gSelectionIds.length > 0) {
            if (this.activeView !== null) delete this.activeView;
            this.activeView = null;
        ...
        case this.viewsID.DetailMicroscopicView:
          if (_SingletonConfig.neuronSelected !== -1) {
            if (this.activeView !== null) delete this.activeView;
            this.activeView = null;
            ...
            if (neuronID === null || neuronID === "") {
              ...
              _SingletonConfig.neuronSelected = neuronID;
              if (this.activeView !== null) delete this.activeView;
              this.activeView = null;
              this.activeView = new MSP.DetailMicroscopicView();
              this.activeView.generateDetailMicroscopicView();
  ...
  _openTab: function (tabName, buttonIndex) {
    ...
    return function () {
      ...
      for (i = 0; i < y.length; i++) {
        $(".navOptionsButton").eq(i).removeClass("active");
        $(".navOptionsButton img").eq(i).attr("src", $(".navOptionsButton img").eq(i)
                                                                               .attr("src")
                                                                               .replace("B.png", ".png"));
      }

      $(".navOptionsButton").eq(buttonIndex).addClass("active");
      $(".navOptionsButton img").eq(buttonIndex).attr("src", $(".navOptionsButton img").eq(buttonIndex)
                                                                                       .attr("src")
                                                                                       .replace(".png", "") + "B.png");
      if (tabName === 'config') {
      ...
      document.getElementById("closeTabs").src = "./assets/images/close.png"
    };
  },
  ...
  resize: function () {
    ...
    _SingletonConfig.height = $(window).height() - $("#jqxBottomControls").outerHeight() -
                              $("#colorSampleBand").outerHeight();
    _SingletonConfig.width = $(window).width() - $("#icon-bar").outerWidth();
    ...
    var width = ($('#control1').outerWidth() + $('#control2').outerWidth() + $('#control4').outerWidth() +
                 $('#control5').outerWidth()) * 1.1;
  ...
  loadLocalFiles: function () {
    ...
    if (Object.keys(self.simulationFiles).length === 6) {
      ...
      _SingletonConfig.recalculatePosScales();
    }
    else {
  ...
  createSampleBandColor: function () {
    ...
    var z = d4.scaleSequential(d4["interpolate" + _SingletonConfig.calciumScheme]);
    ...
    svgContainer.append("rect")
      ...
      .attr("y", scaleYPos)
      .attr("x", marginSide)
      .attr("width", _SingletonConfig.width - marginSide * 2)
      .attr("fill", "url(#scaleGradient)");

    var xI = d3.scale.linear()
      .range([0, _SingletonConfig.width - marginSide * 2])
