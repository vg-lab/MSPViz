...
MSP.MacroscopicViewCanvas.prototype = {
  ...
  mouseDown: function (e) {
    ...
    if (_SingletonConfig.shiftKey) {
      ...
      var color = self.hiddenCanvasContext.getImageData(
        parseInt((d3.mouse(this)[0] - self.translateX) / self.scale, 10),
  ...
  mouseMove: function (e) {
    ...
    if (self.mouseClickDown && _SingletonConfig.shiftKey) {
      ...
      _SimulationData.gNeurons.forEach(function (d) {
        ...
        d.selected = d.previouslySelected ^ (scaledSelectionRect.xStart <= posX
                                             && posX < scaledSelectionRect.xEnd
                                             && scaledSelectionRect.yStart <= posY
    ...
    var color = self.hiddenCanvasContext.getImageData(parseInt((d3.mouse(this)[0] - self.translateX) / self.scale, 10),
                                                      parseInt((d3.mouse(this)[1] - self.translateY) / self.scale, 10),
                                                      1, 1).data;
      ...
      function (d, i) {
        ...
        if (d.draw
          ...
          d.data.forEach(function (d, i) {
            ...
            context.moveTo(self.neuronsPosX[_SimulationData.gNeurons[d[0]].index],
                           self.neuronsPosY[_SimulationData.gNeurons[d[0]].index]);
            context.lineTo(self.neuronsPosX[_SimulationData.gNeurons[d[1]].index],
                           self.neuronsPosY[_SimulationData.gNeurons[d[1]].index]);
            context.stroke();
            canvas_arrow(context, self.neuronsPosX[_SimulationData.gNeurons[d[0]].index],
                         self.neuronsPosY[_SimulationData.gNeurons[d[0]].index],
                         self.neuronsPosX[_SimulationData.gNeurons[d[1]].index],
                         self.neuronsPosY[_SimulationData.gNeurons[d[1]].index]);
            context.globalAlpha = 1;
