...
MSP.DetailMicroscopicView.prototype = {
  ...
  generateDetailMicroscopicView: function (rootID) {
    ...
    var svg = d3.select("#renderArea")
                ...
                .append("g")
                .attr("transform",
                      "translate(" + this.radialLayoutWidth * 0.5 + "," + _SingletonConfig.height * 0.5 + ")"
                )
                      ...
                      .on("mouseover", function () {
                        ...
                        d3.select("#tooltip")
                          ...
                          .html(
                            ...
                            + "</b><br> CaC= <b>" +
                            _SimulationData.gNeuronsDetails[_SingletonConfig.neuronSelected].Calcium[lIndex]
                            + "</b><br>  SE_E: <b>" + ENode
                            + "</b> SE_I: <b>" + INode
                            + "</b> SE_A: <b>" + ANode + "</b>"
                          );
        ...
        .style("fill", function (d) {
          ...
          else if (d.id == "Axonal") //Axon
          {
            ...
            if (_SimulationData.gNeurons[_SingletonConfig.neuronSelected].NAct == "E")
              lTmpColor = new KolorWheel(_SingletonConfig.EColor).getRgb();
            else
              lTmpColor = new KolorWheel(_SingletonConfig.IColor).getRgb();
  ...
  recalculateChilds: function (pParentId, pActNumChilds) {
    ...
    if (_SingletonConfig.SEViewSelector !== 1) {
      if (pParentId === "Axonal") {
        if (typeof (_SimulationData.gConnectivity.EI[_SimulationData.steps[_SimulationController.actSimStep]]) !==
            "undefined")
        ...
        if (typeof (_SimulationData.gConnectivity.EE[_SimulationData.steps[_SimulationController.actSimStep]]) !==
            "undefined")
        ...
        if (typeof (_SimulationData.gConnectivity.IE[_SimulationData.steps[_SimulationController.actSimStep]]) !==
            "undefined")
          _SimulationData.gConnectivity.IE[_SimulationData.steps[_SimulationController.actSimStep]].forEach(
            function (d) {
              if (d[0] === lId) idsList.push(d[1])
            });
        if (typeof (_SimulationData.gConnectivity.II[_SimulationData.steps[_SimulationController.actSimStep]]) !==
            "undefined")
  ...
  updateCalcium: function () {
    ...
    this.soma.style("fill", function () {
      ...
      if (_SimulationData.gNeurons[_SingletonConfig.neuronSelected].NAct == "E") {
        var lVal = _SimulationData.CaEScale(
          _SimulationData.gNeuronsDetails[_SingletonConfig.neuronSelected].Calcium[lIndex]);
        return lVal;
      }
      else {
        var lVal = _SimulationData.CaIScale(
          _SimulationData.gNeuronsDetails[_SingletonConfig.neuronSelected].Calcium[lIndex]);
        return lVal;
      }
    });
