...
MSP.MicroscopicView.prototype = {
  ...
  generateMicroscopicView: function () {
                   ...
                   .on("brush", function (d) {
                     if (_SimulationController.view.selecting) {
                       ...
                       _SingletonConfig.gSelectionIds.forEach(function (i) {
                         ...
                         var x = (_SingletonConfig.xScale(d.PosX) - self.zoombehavior.translate()[0]) /
                                 self.zoombehavior.scale();
                         var y = (_SingletonConfig.yScale(d.PosY) - self.zoombehavior.translate()[1]) /
                                 self.zoombehavior.scale();
    ...
    this.paths = this.pies
                     ...
                     .style("fill", function (d, i) {
                              ...
                              if ((self.selectedMicro.length === 0 && !_SimulationFilter.gNeuronsFilterB[lRealNeuId]) ||
                                  (self.selectedMicro.length !== 0 && !_SimulationData.gNeurons[lRealNeuId].selectedM)) {
                                return "#434343";
                              }
                              ...
                              if (i == 1) {
                                ...
                                var lTmpColor;
                                if (_SimulationData.gNeurons[lRealNeuId].NAct == "E") lTmpColor = new KolorWheel(
                                  _SingletonConfig.EColor).getRgb();
                     ...
                     .on("mousemove", function (d, i) {
                       ...
                       var tooltipX = d3.mouse(d3.select('body').node())[0] +
                                      self.tooltipMarginRatio * _SingletonConfig.width;
                       var tooltipY = d3.mouse(d3.select('body').node())[1] +
                                      self.tooltipMarginRatio * _SingletonConfig.height;
                       var tooltipWidth = $("#tooltip").outerWidth();
                       var tooltipHeight = $("#tooltip").outerHeight();
  ...
  updateCalcium: function () {
    ...
    this.shapes.style("fill", function (d) {
      if ((self.selectedMicro.length === 0 && !_SimulationFilter.gNeuronsFilterB[d]) ||
          (self.selectedMicro.length !== 0 && !_SimulationData.gNeurons[d].selectedM)) {
        return "#434343";
      }
      ...
      if (_SimulationData.gNeurons[d].NAct == "E") {
        ...
        return lVal;
               ...
               if (!_SimulationFilter.gNeuronsFilterB[d] ||
                   (!_SimulationData.gNeurons[d].selectedM && self.selectedMicro.length > 0)) {
                 return 0.1
               }
               else return 1
    ...
    this.paths =
      this.pies
          ...
          .attr("fill-opacity", function (d, i) {
                  ...
                  var lIndirectNeuId = parseInt(elm.attr('id') / 3);
                  var lRealNeuId = _SingletonConfig.gSelectionIds[lIndirectNeuId];
                  if (!_SimulationFilter.gNeuronsFilterB[lRealNeuId] ||
                      (!_SimulationData.gNeurons[lRealNeuId].selectedM && self.selectedMicro.length > 0)) {
                    return 0.1
                  }
                  else return 1
          ...
          function (d) {
            ...
            switch (_SingletonConfig.SEViewSelector) {
              case 0:
                ...
                if (lArcId == 0) {
                  d.outerRadius = self.GlScale(
                    _SimulationData.gNeuronsDetails[lRealNeuId].DeSeEA[(lIndex - lPrevInc) %
                                                                       _SimulationData.steps.length]);
                  d.foo = self.GlScale(
                    _SimulationData.gNeuronsDetails[lRealNeuId].DeSeEA[lIndex % _SimulationData.steps.length]);
                }
                //Axon elements
                if (lArcId == 1) {
                  d.outerRadius = self.GlScale(_SimulationData.gNeuronsDetails[lRealNeuId].AxSeA[(lIndex - lPrevInc) %
                                                                                                   _SimulationData.steps.length]);
                  d.foo = self.GlScale(
                    _SimulationData.gNeuronsDetails[lRealNeuId].AxSeA[lIndex % _SimulationData.steps.length]);
                }
                //Dendritic inhibitory
                if (lArcId == 2) {
                  d.outerRadius = self.GlScale(
                    _SimulationData.gNeuronsDetails[lRealNeuId].DeSeIA[(lIndex - lPrevInc) %
                                                                       _SimulationData.steps.length]);
                  d.foo = self.GlScale(
                    _SimulationData.gNeuronsDetails[lRealNeuId].DeSeIA[lIndex % _SimulationData.steps.length]);
                }
                break;
              ...
              case 1:
                ...
                if (lArcId == 0) {
                  d.outerRadius = self.GlScale(
                    _SimulationData.gNeuronsDetails[lRealNeuId].DeSeEV[(lIndex - lPrevInc) %
                                                                       _SimulationData.steps.length]);
                  d.foo = self.GlScale(
                    _SimulationData.gNeuronsDetails[lRealNeuId].DeSeEV[lIndex % _SimulationData.steps.length]);
                }
                //Axon elements
                if (lArcId == 1) {
                  d.outerRadius = self.GlScale(_SimulationData.gNeuronsDetails[lRealNeuId].AxSeV[(lIndex - lPrevInc) %
                                                                                                   _SimulationData.steps.length]);
                  d.foo = self.GlScale(
                    _SimulationData.gNeuronsDetails[lRealNeuId].AxSeV[lIndex % _SimulationData.steps.length]);
                }
                //Dendritic inhibitory
                if (lArcId == 2) {
                  d.outerRadius = self.GlScale(
                    _SimulationData.gNeuronsDetails[lRealNeuId].DeSeIV[(lIndex - lPrevInc) %
                                                                       _SimulationData.steps.length]);
                  d.foo = self.GlScale(
                    _SimulationData.gNeuronsDetails[lRealNeuId].DeSeIV[lIndex % _SimulationData.steps.length]);
                }
                break;
              ...
              case 2:
                ...
                if (lArcId == 0) {
                  d.outerRadius = self.GlScale(
                    _SimulationData.gNeuronsDetails[lRealNeuId].DeSeEC[(lIndex - lPrevInc) %
                                                                       _SimulationData.steps.length]);
                  d.foo = self.GlScale(
                    _SimulationData.gNeuronsDetails[lRealNeuId].DeSeEC[lIndex % _SimulationData.steps.length]);
                }
                //Axon elements
                if (lArcId == 1) {
                  d.outerRadius = self.GlScale(_SimulationData.gNeuronsDetails[lRealNeuId].AxSeC[(lIndex - lPrevInc) %
                                                                                                   _SimulationData.steps.length]);
                  d.foo = self.GlScale(
                    _SimulationData.gNeuronsDetails[lRealNeuId].AxSeC[lIndex % _SimulationData.steps.length]);
                }
                //Dendritic inhibitory
                if (lArcId == 2) {
                  d.outerRadius = self.GlScale(
                    _SimulationData.gNeuronsDetails[lRealNeuId].DeSeIC[(lIndex - lPrevInc) %
                                                                       _SimulationData.steps.length]);
                  d.foo = self.GlScale(
                    _SimulationData.gNeuronsDetails[lRealNeuId].DeSeIC[lIndex % _SimulationData.steps.length]);
                }
                break;
  ...
  transformShapes: function (d) {
    return "translate(" + _SingletonConfig.xScale(_SimulationData.gNeurons[d].PosX) + "," +
           _SingletonConfig.yScale(_SimulationData.gNeurons[d].PosY) + ")";
  },
