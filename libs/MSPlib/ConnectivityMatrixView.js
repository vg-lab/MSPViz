...
MSP.ConnectivityMatrixView.prototype = {
    ...
    generateConnectivityMatrixView: function () {
        ...
        _SigletonConfig.svg.on('mousedown', this.mouseDown.bind(this, this), false);
        _SigletonConfig.svg.on('mousemove', this.mouseMove.bind(this, this), false);
    ...
    draw: function (x, y) {
        ...
        if (typeof x !== "undefined" && typeof y !== "undefined") {
            ...
            context.rect(-(this.translateX / this.scale),
                (y * (this.squareSize + this.squareMargin)) + this.squareMargin, _SigletonConfig.width / this.scale,
                this.squareSize);
            context.fill();
            context.beginPath();
            context.rect((x * (this.squareSize + this.squareMargin)) + this.squareMargin,
                -(this.translateY / this.scale), this.squareSize, _SigletonConfig.height / this.scale);
            context.fill();
        }
        ...
        data.forEach(function (d) {
                ...
                if (d.draw && typeof (d.data) !== "undefined") {
                    ...
                    d.data.forEach(function (d) {
                        ...
                        context.beginPath();
                        context.rect((_SimulationData.gNeurons[d[1]].index * (self.squareSize + self.squareMargin)) + self.squareMargin,
        ...
        context.beginPath();
        context.rect(-(this.translateX / this.scale),
            -((this.translateY) / this.scale), _SigletonConfig.width / this.scale, this.axisSize / this.scale);
        context.fill();


        context.beginPath();
        context.rect(-(this.translateX / this.scale),
            -((this.translateY) / this.scale), this.axisSize / this.scale, _SigletonConfig.height / this.scale);
        ...
        context.lineTo(-((this.translateX - this.axisSize - _SigletonConfig.width) / this.scale),
            -((this.translateY - this.axisSize) / this.scale));
        ...
        context.lineTo(-((this.translateX - this.axisSize) / this.scale),
            -((this.translateY - this.axisSize - _SigletonConfig.height) / this.scale));
        context.lineWidth = 1 / this.scale;
        context.stroke();
    ...
    mouseDown: function (self) {
        if (parseInt(d3.mouse(_SigletonConfig.svg.node())[0]) <= self.axisSize && parseInt(d3.mouse(_SigletonConfig.svg.node())[0]) <= self.axisSize) {
    ...
    mouseMove: function (self) {
        var color = rgbToHex(self.context.getImageData(parseInt(d3.mouse(_SigletonConfig.svg.node())[0]), parseInt(d3.mouse(_SigletonConfig.svg.node())[1]), 1, 1).data);
        if (color === _SigletonConfig.EEColor || color === _SigletonConfig.IIColor
            ...
            self.draw(parseInt(parseInt((d3.mouse(_SigletonConfig.svg.node())[0] - self.translateX) / self.scale, 10) / (self.squareSize + self.squareMargin)), parseInt(parseInt((d3.mouse(_SigletonConfig.svg.node())[1] - self.translateY) / self.scale, 10) / (self.squareSize + self.squareMargin)));
            ...
            d3.select("#tooltip")
                ...
                .style("top", d3.mouse(_SigletonConfig.svg.node())[1] + 10 + "px")
                .html(
                    "From: <b>" + _SimulationFilter.orderIndex[parseInt(parseInt((d3.mouse(_SigletonConfig.svg.node())[1] - self.translateY) / self.scale, 10) / (self.squareSize + self.squareMargin))]
                    + "</b><br> To: <b>" + _SimulationFilter.orderIndex[parseInt(parseInt((d3.mouse(_SigletonConfig.svg.node())[0] - self.translateX) / self.scale, 10) / (self.squareSize + self.squareMargin))] + "</b>"
                );
    ...
    zoom: function (self) {
        if (!self.resetZoom) {
            self.translateX = Math.max(Math.min(d3.event.translate[0], self.axisSize), ((-_SimulationData.gNeurons.length * (self.squareSize + self.squareMargin) * d3.event.scale) + _SigletonConfig.width));
            self.translateY = Math.max(Math.min(d3.event.translate[1], self.axisSize), ((-_SimulationData.gNeurons.length * (self.squareSize + self.squareMargin) * d3.event.scale) + _SigletonConfig.height));
